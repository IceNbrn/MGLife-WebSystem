...
if(isset($_GET["id"])){
    ...
    $sql = "SELECT * FROM reportpessoa WHERE id = $Id";
...
<head>
    ...
    <title>MGLife | Atualizar Fugitivo</title>
...
<div class="row">
    <div class="container">
        <h2>Atualizar Fugitivo</h2>
        <form role="form" id="formLogin" method="post" action="atualizarf.php?id=<?=$Id?>">
            ...
            if(isset($_POST["btnconcluir"])){
                if(empty($_POST["sl_status"]) && $_POST["sl_status"] != 0){
                    ...
                    $patrulha->UpdateFugitivo($Id,$status);
                }
            }
            ?>
        </form>
